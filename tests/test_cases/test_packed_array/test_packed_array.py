...
async def force_freeze_release_indexed_dimension(dut):
    """Force, freeze, and release a driven indexed packed dimension."""

    input_part = dut.i_packed_3d[1][1]
    output_part = dut.o_packed_3d[1][1]

    input_part.value = 1
    await Timer(1)
    output_part.value = Force(0xE)
    input_part.value = 2
    await Timer(1)
    assert output_part.value == 0xE

    output_part.value = Release()
    input_part.value = 3
    await Timer(1)
    assert output_part.value == 3

    output_part.value = Freeze()
    input_part.value = 4
    await Timer(1)
    assert output_part.value == 3
...
async def edge_clk(dut):
    """Drive and wait for a edge on a clock."""
    sig = dut.clk
...
async def edge_2d_bit(dut):
    """Drive and wait for a edge on one bit of a 2-D packed array."""
